...
def supervised_training(x,
    ...
    predictions, y_test_back = transform_pred_to_original_units(pred,y_test,target_transform,target_means,target_stds,target_ppm_concentrations)

    if not bootstrap:
        print(method+' Mean Absolute Error: ', mare(y_test_back,predictions))

    return trained_regressor, predictions, y_test_back, target_means, target_stds

def transform_pred_to_original_units(pred,y_test,target_transform,target_means,target_stds,target_ppm_concentrations):
    if target_transform=='standardize':
    ...
    return predictions, y_test_back
